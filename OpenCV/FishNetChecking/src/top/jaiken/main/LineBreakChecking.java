...
public class LineBreakChecking {
	...
	public boolean isLineBreak(Mat mat) {
		...
		dst = src.clone();
		/**
		 * 侵蚀操作: Mat kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT,
		 * new Size((3 * 3) + 1, (3 * 3) + 1)); Imgproc.erode(src, dst, kernel);
		 */

		/**
		 * 膨胀操作 :Mat dst = new Mat(); Mat kernel =
		 * Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size((3 * 2) +
		 * 1, (3* 2) + 1)); Imgproc.dilate(src, dst, kernel);
		 */

		/**
		 * 双边滤镜 Imgproc.bilateralFilter(dst, dst, 5, 80, 80,
		 * Core.BORDER_DEFAULT);
		 */
		Mat canny = new Mat();
